...
namespace rst
{
    ...
    public:
		...
		void set_test(std::function<int(fragment_shader_payload)> f);
        ...
        void draw(std::vector<Triangle *> &TriangleList, bool msaa = false);
    ...
    private:
		...
		void rasterize_triangle_msaa(const Triangle& t, const std::array<Eigen::Vector3f, 3>& world_pos);

		//template<class T>
		//T interpolation(T t[], int size, std::tuple<float, float, float> c, float w_reciprocal, Vector3f w);

		//Vector3f interpolation(Vector3f t[], int size, std::tuple<float, float, float> c, float w_reciprocal, Vector3f w);
    ...
    private:
		...
		std::function<int(fragment_shader_payload)> _f;
	...
	//template<class T>
	//inline T rasterizer::interpolation(T t[], int size, std::tuple<float, float, float> c, float w_reciprocal, Vector3f w)
	//{
	//	T res;
